...
class SequenceRunner:
    ...
    def run(self, poses: List[CalibPose],
            per_pose_cb: Callable[[CalibPose], None],
            settle_s: float = 0.5,
        ...
        for pose in poses:
            if len(pose.joints) != len(joint_names):
                self._node.get_logger().warn(
                    f'Pose {pose.name}: expected {len(joint_names)} joint '
                    f'values, got {len(pose.joints)}')
                continue
            self._node.get_logger().info(f'Moving to pose {pose.name}')
